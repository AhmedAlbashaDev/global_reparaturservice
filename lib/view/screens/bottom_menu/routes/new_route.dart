...
final selectTechnicianLater = StateProvider.autoDispose<bool?>((ref) => true);
...
class NewRouteScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState createState() => _NewRouteScreenState(isUpdate: isUpdate , routesModel: routesModel);
}

class _NewRouteScreenState extends ConsumerState<NewRouteScreen>
    with TickerProviderStateMixin {

  _NewRouteScreenState({this.isUpdate , this.routesModel});
        ...
        child: Stack(
          children: [
            ...
            Column(
              children: [
                CustomAppBar(
                  title: isUpdate == true ? 'update_route'.tr() : 'new_route'.tr(),
                ),
                Expanded(
                    child: Padding(
                  padding:
                                ...
                                children: [
                                  AutoSizeText.rich(
                                    TextSpan(text: 'selected_orders'.tr(), children:  [
                                      TextSpan(
                                          text: ' (${ref.watch(selectedOrdersToNewOrder).length - (routesModel?.orders?.length ?? 0) })',
      ...
      floatingActionButton: SizedBox(
        ...
        child: CustomButton(
          onPressed: ref.watch(selectedOrdersToNewOrder).isEmpty ? null :
              () {
                  if(isUpdate == true){
                    //update route with new orders using ref
                    AwesomeDialog(
                        context: context,
                        dialogType: DialogType.question,
                        animType: AnimType.rightSlide,
                        title: 'update_route'.tr(),
                        desc: 'Are you sure you want to add these orders to the route'.tr(),
                        autoDismiss: false,
                        ...
                        btnOk: CustomButton(
                          onPressed: () {
                            Navigator.of(context).pop();
                            Navigator.of(context).pop();
                            ref.read(routeViewModelProvider.notifier).update(routeId: routesModel?.id, orders: ref.read(selectedOrdersToNewOrder));
                          },
                          radius: 10,
                          text: 'Yes'.tr(),
                          textColor: Colors.white,
                    ...
                    Navigator.push(context, MaterialPageRoute(builder: (context) =>  ConfirmNewRoute(technician: ref.read(selectTechnicianLater) != true ? ref.read(selectedTechnician) : null)));
                  }
                  // else{
                // Navigator.push(context, MaterialPageRoute(builder: (context) =>  ConfirmNewRoute(technician: ref.read(selectTechnicianLater) != true ? ref.read(selectedTechnician) : null)));
          ...
          text: isUpdate == true ? 'update_route'.tr() : 'next'.tr(),
          textColor: Colors.white,
